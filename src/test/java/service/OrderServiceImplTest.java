...
class OrderServiceImplTest {
    ...
    void testUpdateStatusInvalidOrderId() {
        UUID uuidZczc = UUID.randomUUID(); // Pengganti "zczc" karena harus format UUID
        doReturn(null).when(orderRepository).findById(uuidZczc);

        assertThrows(NoSuchElementException.class, () ->
                orderService.updateStatus(uuidZczc, OrderStatus.SUCCESS.getValue()));
    ...
    void testFindByIdIfIdNotFound() {
        UUID uuidZczc = UUID.randomUUID();
        doReturn(null).when(orderRepository).findById(uuidZczc);

        assertNull(orderService.findById(uuidZczc));
    }
    ...
    void testFindAllByAuthorIfAllLowercase() {
        Order order = orders.get(1);
        String lowercaseAuthor = order.getAuthor().toLowerCase();
